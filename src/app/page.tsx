...
// Interface pour les particules fiscales
interface FiscalParticle {
  id: number;
  x: number;
  y: number;
  size: number;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fiscalParticles: FiscalParticle[] = [];
    ...
    // Créer des particules fiscales discrètes
    const createFiscalParticles = () => {
      for (let i = 0; i < 12; i++) {
        fiscalParticles.push({
          id: i,
          x: Math.random() * canvas.width,
          y: Math.random() * canvas.height,
          size: Math.random() * 10 + 5,
          symbol: fiscalSymbols[Math.floor(Math.random() * fiscalSymbols.length)],
    ...
    // Dessiner une particule fiscale
    const drawFiscalParticle = (particle: FiscalParticle) => {
      ctx.save();
      ctx.translate(particle.x, particle.y);
      ctx.rotate(particle.rotation);
      ctx.globalAlpha = particle.opacity;
      
      // Dégradé pour la particule
    ...
    const animate = (timestamp: number) => {
      ...
      // Animer les particules fiscales
      fiscalParticles.forEach(particle => {
        ...
        if (particle.opacity > 0.3 || particle.opacity < 0.1) {
