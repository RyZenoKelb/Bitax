...
// Metadata - remplace les metadata dans l'ancien app/layout.tsx
...
// Layout principal qui remplace _app.tsx et intègre _document.tsx
export default function RootLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <html lang="fr" suppressHydrationWarning className={theme}>
      <head>
          ...
          <DevModeProvider>
            ...
            <div className={`min-h-screen flex ${isLoaded ? 'opacity-100' : 'opacity-0'} transition-opacity duration-500`}>
              ...
              >
                ...
                {/* Bouton toggle sidebar - invisible mais gardé pour fonctionnalité */}
                  ...
                  <nav className="px-2 pt-5 pb-3 space-y-1">
                    {navLinks.map((link) => (
                      <Link 
                        ...
                        onClick={() => setIsMobileMenuOpen(false)}
                      >
                        <div className={`${pathname === link.href ? 'text-primary-400' : 'text-gray-400'}`}>
                          {link.icon}
                        </div>
